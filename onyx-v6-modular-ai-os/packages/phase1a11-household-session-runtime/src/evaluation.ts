...
const assuranceRank = (value: string): number => ({ low: 1, standard: 2, strong: 3 }[value as "low" | "standard" | "strong"] ?? 0);
const effectiveAssurance = (session: SessionRecord): "low" | "standard" | "strong" => {
  if (session.authenticationAssurance === "unknown") return "unknown" as never;
  if (session.status === "elevated") return "strong";
  return session.authenticationAssurance as "low" | "standard" | "strong";
};
...
export function evaluateSession(input: SessionEvaluationInput): SessionEvaluationResult {
  ...
  if (input.session.authenticationAssurance === "unknown") return invalid("INVALID_AUTHENTICATION_ASSURANCE", input);
  const sessionAssurance = effectiveAssurance(input.session);
  if (input.requiredAssurance && assuranceRank(sessionAssurance) < assuranceRank(input.requiredAssurance)) {
    const result = invalid("STEP_UP_REQUIRED", input);
