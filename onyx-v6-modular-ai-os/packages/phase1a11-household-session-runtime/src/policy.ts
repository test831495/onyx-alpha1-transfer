...
export function validateSessionPolicy(policy: ConcurrentSessionPolicy, expectedVersion: PolicyVersion = "policy-1"): { valid: boolean; technicalReason: string } {
  ...
  if (!Number.isInteger(policy.concurrentSessionLimit) || policy.concurrentSessionLimit < 1 || !["account", "device"].includes(policy.concurrentSessionScope)) return { valid: false, technicalReason: "INVALID_CONCURRENT_SESSION_LIMIT" };
