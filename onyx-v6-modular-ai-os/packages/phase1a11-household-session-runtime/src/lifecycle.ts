...
export function createSession(input: SessionCreationInput): SessionCreationResult {
  ...
  const session: SessionRecord = { sessionId: "session_fixture_001", familyId: "session-family_fixture_001", revision: 1, binding: { accountId: input.identity.account.accountId, householdId: input.identity.householdId, membershipId: input.identity.membership.membershipId, roleId: input.identity.membership.roleId, deviceContextId: input.deviceContextId!, authenticationEventReference: input.authentication.eventReference }, authenticationAssurance: input.authentication.assurance, versions: input.versions, permissionBinding: { permissionCatalogVersion: input.versions.permissionCatalogVersion, roleId: input.identity.membership.roleId, roleVersion: input.versions.roleVersion, membershipId: input.identity.membership.membershipId }, timing: { createdAt: input.currentTime, lastActivityAt: input.currentTime, inactivityDeadline: inactivity, absoluteDeadline: absolute, rotationAt: new Date(now + input.policy.rotationIntervalMs).toISOString() }, status: "active", sharedDevice: input.policy.sharedDeviceRestrictions.classification, audit: { required: input.policy.auditRequired, purpose: "session-lifecycle", }, provenanceReference: input.provenanceReference };
