...
function composeBoundedText(input: DispatcherInput, speaker: FinalSpeaker, mode: FinalResponseMode, offline: boolean): string { if (offline) return "I am operating locally. I can continue using local context and capabilities, but current cloud information is unavailable."; if (mode === "SAFE_LIMITATION") return "I cannot verify that current information from the available evidence."; if (mode === "CLARIFICATION") return "I need a little more context before I can answer safely."; if (mode === "ACTION_PROPOSAL") return input.actionTarget ? "I can propose that action, but approval is required and no action has been taken." : "The action target and scope need clarification before a proposal can be prepared."; if (speaker === "ONYX") return "My recommendation is to use the evidence available, weigh the tradeoff, and choose a reversible next step."; if (speaker === "COUNCIL") return "Both perspectives are eligible for consideration; no authority or execution is granted."; return "Here is a bounded answer from the available context, followed by one practical next step."; }
