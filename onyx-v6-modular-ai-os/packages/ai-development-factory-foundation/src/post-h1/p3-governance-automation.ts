...
export const evaluateP3GovernanceAutomation = (rawInput: unknown): P3GovernanceAutomationResult => {
  ...
  try {
    ...
    const nextGate = unavailableEvidence ? "PROVIDE_CURRENT_EVIDENCE" : targetInvalid ? "RECONCILE_TARGET" : readiness.outcome !== "TECHNICALLY_READY" ? "REMEDIATE_GOVERNANCE_BLOCKERS" : closure.outcome !== "MAIN_CLOSED" ? "OWNER_REVIEW" : "OWNER_REVIEW";
