import { PERSISTENCE_MODES, ACTION_AUTHORITY_CLASSES, ValidationOutcome } from "./lifecycle-vocabulary";
...
export const validateArchitectureConstitution = (input: unknown): ConstitutionResult => {
  ...
  try {
    ...
    if (!Array.isArray(candidate.actions) || candidate.actions.some((action) => action !== "VALIDATE" && !ACTION_AUTHORITY_CLASSES.includes(action as typeof ACTION_AUTHORITY_CLASSES[number]))) return result("FAIL", ["ACTION_CLASS_INVALID"]);
