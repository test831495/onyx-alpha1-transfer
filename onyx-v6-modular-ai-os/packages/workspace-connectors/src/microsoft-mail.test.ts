...
describe("MicrosoftWorkspaceConnector Mail.ReadBasic foundation", () => {
  ...
  it("MAIL-DIAG-001 captures sanitized error details on persistent 401 without leaks", async () => {
    ...
    expect(result.diagnostic.sanitizedDiagnostic?.forceRefresh).toBe(true);
    expect(result.diagnostic.sanitizedDiagnostic?.retryAttempt).toBe(true);
  });

  it("MAIL-DIAG-002 preserves actual audience in Mail diagnostic envelope instead of hardcoding", async () => {
    ...
    const customAudienceToken = makeSyntheticJwt({ aud: "api://custom-audience" });
    const acquireTokenSilent = vi.fn().mockResolvedValue({
      accessToken: customAudienceToken,
      scopes: ["User.Read", "Mail.ReadBasic"],
      account: { homeAccountId: "account", tenantId: "tenant" },
    });
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(emptyMailboxResponse({ value: [] })));
  ...
  it("MAIL-DIAG-003 constructs a refreshed diagnostic envelope when force-refresh fails silently", async () => {
    const acquireTokenSilent = vi.fn()
      .mockResolvedValueOnce(mailToken({ homeAccountId: "account", tenantId: "tenant" }))
      .mockResolvedValueOnce({ accessToken: "", scopes: ["User.Read", "Mail.ReadBasic"], account: { homeAccountId: "account", tenantId: "tenant" } });
    ...
    expect(result.diagnostic.reasonCode).toBe("MAIL_ACCESS_TOKEN_ABSENT");
    expect(result.diagnostic.sanitizedDiagnostic?.forceRefresh).toBe(true);
    expect(result.diagnostic.sanitizedDiagnostic?.retryAttempt).toBe(false);
  });
});