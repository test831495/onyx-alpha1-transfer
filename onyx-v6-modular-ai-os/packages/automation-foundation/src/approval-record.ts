...
export function approvalValid(plan:ExecutionPlan,approval:ApprovalRecord|undefined,now=Date.now()):boolean{return Boolean(approval&&approval.planId===plan.id&&approval.scopeHash===plan.scopeHash&&Date.parse(approval.expiresAt)>now)}
