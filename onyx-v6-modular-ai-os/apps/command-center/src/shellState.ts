...
export function resolveShellIntent(raw: string): ShellIntent | null {
  ...
  const match = text.match(/^(open|show|go to|focus|minimize|close|hide|exit|dismiss)\s+(.+)$/);
  if (match) {
    const verb = match[1] ?? "open";
    const targetText = (match[2] ?? "").trim().replace(/^the\s+/, "").replace(/\s+app$/, "");
    const appId = SHELL_APP_ALIASES[targetText] ?? SHELL_APP_ALIASES[targetText.replace(/\s+/g, " ")];
    ...
    if (verb === "close" || verb === "hide" || verb === "exit" || verb === "dismiss") return { type: "CLOSE_APP", appId };
  }
