...
export type AuthenticatedFunctionClientDependencies = {
  readonly fetcher?: typeof fetch;
  readonly getAccessToken?: (scopes: readonly string[], options?: { includeProfileScopes?: boolean }) => Promise<string>;
  readonly interactiveSignIn?: () => Promise<void>;
  readonly runtimeEnv?: Pick<typeof microsoftRuntimeEnv, "ONYX_AUTH_SCOPE">;
  readonly randomUUID?: () => string;
};

const isOnyxInteractionRequired = (error: unknown): boolean => {
  const cause = error instanceof Error ? error.cause : undefined;
  const values = [error, cause];
  return values.some((value) => {
    if (!value || typeof value !== "object") return false;
    const candidate = value as { errorCode?: unknown; message?: unknown; name?: unknown; code?: unknown };
    return [candidate.errorCode, candidate.message, candidate.name, candidate.code].some((part) => typeof part === "string" && /interaction[_ -]?required|login[_ -]?required|not connected|not authenticated/i.test(part));
  });
};

export const resolveOnyxServerAuthorityScope = (runtimeEnv: Pick<typeof microsoftRuntimeEnv, "ONYX_AUTH_SCOPE"> = microsoftRuntimeEnv): string => {
  const scope = runtimeEnv.ONYX_AUTH_SCOPE?.trim();
  if (!scope) throw new Error("ONYX Server Authority scope is not configured.");
  return scope;
};

async function acquireOnyxServerAuthorityToken(dependencies: AuthenticatedFunctionClientDependencies = {}, retried = false): Promise<string> {
  const scope = resolveOnyxServerAuthorityScope(dependencies.runtimeEnv ?? microsoftRuntimeEnv);
  const getAccessToken = dependencies.getAccessToken ?? ((scopes, options) => microsoft.getAccessToken(scopes, options));
  try {
    return await getAccessToken([scope], { includeProfileScopes: false });
  } catch (error) {
    if (retried || !isOnyxInteractionRequired(error)) {
      throw new Error("Sign in to ONYX before connecting Google.");
    }
    const interactiveSignIn = dependencies.interactiveSignIn ?? (() => microsoft.authorizeScopesInteractively([scope]));
    try {
      await interactiveSignIn();
    } catch {
      throw new Error("Sign-in was cancelled. Google remains disconnected.");
    }
    return acquireOnyxServerAuthorityToken(dependencies, true);
  }
}

export async function authenticatedFunctionFetch(input: string, init: RequestInit = {}, dependencies: AuthenticatedFunctionClientDependencies = {}): Promise<Response> {
  const accessToken = await acquireOnyxServerAuthorityToken(dependencies);
...
const googleJson = async <T>(response: Response): Promise<T> => (await response.json()) as T;

const googleFunctionErrorMessage = (response: Response, message: string | undefined): string => {
  if ((response.status === 400 || response.status === 401) && /session|required|rejected|token/i.test(message ?? "")) {
    return "Sign in to ONYX before connecting Google.";
  }
  return message ?? "Google Workspace status is unavailable.";
};

export async function runGoogleWorkspaceAction(action: GoogleAction): Promise<void> {
  return runGoogleWorkspaceActionWithClient(action);
}

export async function runGoogleWorkspaceActionWithClient(action: GoogleAction, dependencies: AuthenticatedFunctionClientDependencies = {}): Promise<void> {
  if (action === "connect" || action === "reconnect") {
    const csrfResponse = await authenticatedFunctionFetch("/.netlify/functions/google-csrf", { method: "GET" }, dependencies);
    const csrf = await googleJson<{ csrfToken?: string; message?: string }>(csrfResponse);
    if (!csrfResponse.ok || !csrf.csrfToken) throw new Error(googleFunctionErrorMessage(csrfResponse, csrf.message));
    const response = await authenticatedFunctionFetch("/.netlify/functions/oauth-google-initiate", {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-csrf-token": csrf.csrfToken,
        "idempotency-key": dependencies.randomUUID?.() ?? crypto.randomUUID(),
      },
      body: JSON.stringify({ reconnect: action === "reconnect" }),
    }, dependencies);
    const payload = await googleJson<{ authorizationUrl?: string; message?: string }>(response);
    if (!response.ok || !payload.authorizationUrl) throw new Error(googleFunctionErrorMessage(response, payload.message));
    window.location.assign(payload.authorizationUrl);
    return;
  }
  const response = await authenticatedFunctionFetch(action === "disconnect" ? "/.netlify/functions/google-workspace-disconnect" : "/.netlify/functions/google-workspace-status", {
    method: action === "disconnect" ? "POST" : "GET",
    ...(action === "disconnect" ? { headers: { "content-type": "application/json" }, body: "{}" } : {}),
  }, dependencies);
  const payload = await googleJson<{ message?: string }>(response);
  if (!response.ok) throw new Error(googleFunctionErrorMessage(response, payload.message));
}
