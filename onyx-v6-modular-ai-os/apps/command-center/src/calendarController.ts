...
}const j:{value?:any[];"@odata.nextLink"?:string}=await r.json();for(const e of j.value??[])events.push({id:e.id,subject:e.sensitivity==="private"&&!e.subject?"Private appointment":e.subject||"Untitled event",start:e.start.dateTime,end:e.end.dateTime,isAllDay:e.isAllDay,isCancelled:e.isCancelled,showAs:e.showAs,location:e.location?.displayName,organizer:e.organizer?.emailAddress?.name,isOnlineMeeting:Boolean(e.isOnlineMeeting),joinUrl:e.onlineMeeting?.joinUrl??e.onlineMeetingUrl,sensitivity:e.sensitivity});next=j["@odata.nextLink"];}return analyzeCalendar(events,offset===0?"today":"tomorrow",zone,start,end,new Date())}
