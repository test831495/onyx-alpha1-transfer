    ...
    <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(min(220px,100%),1fr))", gap:"0.75rem" }}>
      {snapshot.providers.map(provider => {
        ...
        return <article key={provider.provider} className="glass-surface" style={{ padding:"0.85rem", borderRadius:"1rem", display:"grid", gap:"0.6rem" }}>
          ...
          {isMicrosoft && <MicrosoftFilesPanel available={provider.state === "connected" && provider.capabilities.some((capability) => capability.id === "files" && capability.enabled)} onRead={async (continuation) => (await loadMicrosoftOneDriveRoot(continuation)).listing} onWriteTest={runBoundedMicrosoftOneDriveTest} />}
