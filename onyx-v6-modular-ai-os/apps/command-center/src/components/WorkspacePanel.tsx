...
export function WorkspacePanel({ snapshot, busy, onConnect, onReconnect, onDisconnect, onRefresh }: { snapshot: WorkspaceSnapshot; busy: boolean; onConnect: () => void; onReconnect: () => void; onDisconnect: () => void; onRefresh: () => void; }) {
    ...
    <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(min(220px,100%),1fr))", gap:"0.75rem" }}>
      {snapshot.providers.map(provider => {
        ...
        return <article key={provider.provider} className="glass-surface" style={{ padding:"0.85rem", borderRadius:"1rem", display:"grid", gap:"0.6rem" }}>
          ...
          {isMicrosoft && <div style={{ display:"flex", gap:"0.5rem", flexWrap:"wrap" }}>{provider.state === "connected" ? <><button type="button" onClick={onReconnect} disabled={busy} aria-label="Reconnect Microsoft workspace" title={busy ? "Reconnect is unavailable while status is loading." : "Reconnect workspace"}>Reconnect</button><button type="button" onClick={onDisconnect} disabled={busy} aria-label="Disconnect Microsoft workspace" title={busy ? "Disconnect is unavailable while status is loading." : "Disconnect workspace"}>Disconnect</button></> : <button type="button" onClick={onConnect} disabled={busy || provider.state === "unconfigured"} aria-label={provider.state === "unconfigured" ? "Connect Microsoft service is unavailable because it is not connected" : "Connect Microsoft workspace"} title={disabledReason ?? "Connect Microsoft workspace"}>Connect</button>}<button type="button" onClick={onRefresh} disabled={busy} aria-label="Refresh Microsoft workspace status" title={busy ? "Refresh is unavailable while status is loading." : "Refresh workspace status"}>Refresh Status</button></div>}
        </article>;
      })}
    </div>
  </section>;
