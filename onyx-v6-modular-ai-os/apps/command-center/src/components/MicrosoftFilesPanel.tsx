...
import type { SharePointResolution } from "@onyx/workspace-connectors";
...
export interface MicrosoftFilesPanelProps {
  ...
  readonly onReadSharePoint?: (driveId: string, itemId: string, continuation?: string) => Promise<FolderListingProjection>;
...
export function MicrosoftFilesPanel({
  ...
  const resolvedState = sharePointState ?? capabilityState(sharePointAccountKind, sharePointAvailable);
  ...
  const openSharePointFolder = async (drive: DriveProjection, itemId: string, continuation?: string) => {
    ...
    setBusy(true);
    try { setSelectedLibrary(drive); setSharePointListing(await onReadSharePoint(drive.driveId, itemId, continuation)); }
