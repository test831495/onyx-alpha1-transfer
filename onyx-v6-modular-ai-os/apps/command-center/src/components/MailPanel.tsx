...
export function MailPanel({
  ...
  return (
    <section id="panel-mail" className="glass-surface" style={{ margin: "1rem", padding: "1rem", borderRadius: "1.25rem", display: "grid", gap: ".8rem" }}>
      ...
      {diagnosticsEnabled && runtimeTrace && <details open style={{ border: "1px solid rgba(101,217,239,.35)", padding: ".6rem", borderRadius: ".5rem" }}><summary>Diagnostic · privacy-safe</summary><dl><dt>Build</dt><dd>{runtimeTrace.buildIdentity.sha} · {runtimeTrace.buildIdentity.context} · {runtimeTrace.buildIdentity.version}</dd><dt>Stage</dt><dd>{runtimeTrace.stage}</dd><dt>Reason</dt><dd>{runtimeTrace.reasonCode ?? "UNSPECIFIED"}</dd><dt>Status</dt><dd>{runtimeTrace.statusClass}</dd><dt>Token stage</dt><dd>{runtimeTrace.tokenStage}</dd><dt>Binding</dt><dd>{runtimeTrace.accountBindingEvidence.bindingDecision}</dd><dt>Envelope</dt><dd>{runtimeTrace.responseEnvelopeClass}</dd><dt>Items</dt><dd>{runtimeTrace.normalizedItemCount} normalized · {runtimeTrace.rejectedItemCount} rejected</dd></dl><button type="button" onClick={onTraceClear}>Clear diagnostic</button></details>}
    </section>
  );
}